...
public class Database implements VariableSpace, LoggingObjectInterface {
  ...
  public PreparedStatement prepareSQL( String sql, boolean returnKeys ) throws KettleDatabaseException {
    ...
    try {
      if ( returnKeys && supportsAutoGeneratedKeys ) {
        return connection.prepareStatement( stripCR( sql ), Statement.RETURN_GENERATED_KEYS );
      } else {
    	  try {
    		  
    		  return connection.prepareStatement( stripCR( sql ) );
    	  } catch ( SQLException ex ) {
    		  log.logDetailed( "#######################sql:###########"  + sql );
    		  log.logDetailed( "#######################Couldn't prepare statement:###########"  + ex );
    	    }
      }
    } catch ( SQLException ex ) {
  ...
  public Long getNextSequenceValue( String schemaName, String sequenceName, String keyfield )
    ...
    try {
      if ( pstmt_seq == null ) {
        pstmt_seq =
          connection.prepareStatement( databaseMeta.getSeqNextvalSQL( stripCR( schemaSequence ) ) );
      }
      ResultSet rs = null;
      try {
      ...
      if ( params != null ) {
        ...
        pstmt =
          connection.prepareStatement(
            stripCR( sql ), ResultSet.TYPE_FORWARD_ONLY, ResultSet.CONCUR_READ_ONLY );
        ...
        log.snap( Metrics.METRIC_DATABASE_EXECUTE_SQL_START, databaseMeta.getName() );
        res = sel_stmt.executeQuery( stripCR( sql ) );
        log.snap( Metrics.METRIC_DATABASE_EXECUTE_SQL_STOP, databaseMeta.getName() );
      }
  ...
  public RowMetaInterface getQueryFieldsFromPreparedStatement( String sql ) throws Exception {
    ...
    try {
      preparedStatement =
          connection.prepareStatement( stripCR( sql ), ResultSet.TYPE_FORWARD_ONLY,
    ...
    try {
      if ( ( inform == null
        ...
        try {
          ...
          ResultSet r = sel_stmt.executeQuery( stripCR( sql ) );
          try {
        ...
        PreparedStatement ps = connection.prepareStatement( stripCR( sql ) );
        try {
          if ( param ) {
  ...
  public void setLookup( String schemaName, String tableName, String[] codes, String[] condition, String[] gets,
    ...
    try {
      ...
      try {
        ...
        prepStatementLookup = connection.prepareStatement( stripCR( sql ) );
  ...
  public boolean prepareUpdate( String schemaName, String tableName, String[] codes, String[] condition,
    ...
    try {
      ...
      try {
        ...
        prepStatementUpdate = connection.prepareStatement( stripCR( s ) );
      } catch ( SQLException ex ) {
  ...
  public boolean prepareDelete( String schemaName, String tableName, String[] codes, String[] condition ) {
    try {
      ...
      try {
        ...
        prepStatementUpdate = connection.prepareStatement( stripCR( sql ) );
      } catch ( SQLException ex ) {
        printSQLException( ex );
        return false;
      }
  ...
  public Object[] getLastLogDate( String logtable, String name, boolean job, LogStatus status )
    ...
    try {
      pstmt = connection.prepareStatement( stripCR( sql ) );
